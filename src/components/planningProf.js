...
const PlanningProf = () => {
  ...
  const fetchPlanning = async () => {
    try {
      ...
      if (userUuid) {
        const profResponse = await fetch(`http://localhost:5000/api/prof/${userUuid}`);
        if (!profResponse.ok) throw new Error("Erreur prof");

        const profData = await profResponse.json();
        setMatiere(profData.matiere);
        ...
        const cours = coursData.find(c => c.matiere === profData.matiere);
  ...
  return (
    <div className="eleveAdminTable">
      <h2>Planning du professeur</h2>
      <p>Matière enseignée : {matiere}</p>
